...
//Stroes the lower limit and higher limit between which the question is picked from the database.
int low_limit;
int high_limit;
...
void GeneralInstructions()
{
	...
	printf("There is no time limit for any question.\n");
	printf("On crossing each step, a cheque will be given to you, which in all circumstances will be availed to you.\n\n");
...
int LifeLines(int ques)
{
	...
	if (flip == 1 && fifty == 0)
	{
		printf("You have got only one lifeline left: Flip the Question you can use it whenever you want .\n\n");
		choice = '2';
	}

	if (flip == 0 && fifty == 1)
	{
		printf("You have got only one lifeline left: 50-50 you can use it whenever you want .\n\n");
		choice = '1';
	}
...
void StepQuestion(int ques)
{
	...
	printf("If you will answer the question correctly, then %s will be fixed to you.\n", Money(ques));
	...
	if (cond != 'Q' && cond != 'q')
	{
		...
		while (ques <= 13)
		{
			...
			if (ques == 13)
			{
				...
				printf("Answer the questiuon only if you are 100% sure.\n");
